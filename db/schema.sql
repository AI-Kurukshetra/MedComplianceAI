-- ============================================================
-- MedCompliance AI — Complete Schema
-- Run this first in Supabase SQL Editor (service-role / postgres)
-- ============================================================
...
-- Module assignments (learner ↔ module)
...
create table if not exists public.notifications (
  ...
  user_id         uuid        not null references public.profiles(id)      on delete cascade,
  kind            text        not null
                                check (kind in ('due_soon','overdue','renewal','assignment','achievement')),
  title           text        not null,
...
returns jsonb language sql stable as $$
  select jsonb_build_object('ok',true,'checked_at',now(),'version',version())
$$;
...
begin
  ...
  return query
    ...
    from public.organizations o where o.slug = v_slug limit 1;
end;
$$;

create or replace function public.provision_signup_organization(
  p_name text, p_slug text default null)
returns table(id uuid, slug text, name text)
...
begin
  ...
  v_base_slug := public.normalize_org_slug(
    coalesce(nullif(trim(coalesce(p_slug,'')),''), v_name));
  if v_base_slug = '' then
  ...
  select o.id into v_template_org_id
  from public.organizations o where o.slug = 'demo-clinic' limit 1;
  if v_template_org_id is not null then
    for module_rec in
      select tm.id, tm.title, tm.description, tm.regulation,
             tm.audience_role, tm.estimated_minutes, tm.content_url, tm.is_active
      from public.training_modules tm
      where tm.organization_id = v_template_org_id and tm.is_active = true
      order by tm.created_at asc limit 12
    loop
      insert into public.training_modules
        (organization_id,title,description,regulation,audience_role,
         estimated_minutes,content_url,is_active)
      values (v_org_id,module_rec.title,module_rec.description,module_rec.regulation,
              module_rec.audience_role,module_rec.estimated_minutes,
              module_rec.content_url,module_rec.is_active)
      returning training_modules.id into v_new_module_id;
      for question_rec in
        select q.id, q.body, q.explanation, q.points, q.position
        from public.questions q
        where q.organization_id = v_template_org_id and q.module_id = module_rec.id
        order by q.position asc, q.created_at asc
      loop
        insert into public.questions
          (module_id,organization_id,body,explanation,points,position)
        values (v_new_module_id,v_org_id,question_rec.body,
                question_rec.explanation,question_rec.points,question_rec.position)
        returning questions.id into v_new_question_id;
        insert into public.question_options (question_id,body,is_correct,position)
        select v_new_question_id, qo.body, qo.is_correct, qo.position
        from public.question_options qo
        where qo.question_id = question_rec.id order by qo.position asc;
      end loop;
    end loop;
  end if;
  return query select o.id, o.slug, o.name from public.organizations o where o.id = v_org_id;
end;
$$;
...
begin
  ...
  exception when others then requested_org_id := null;
  end;
  select id into fallback_org_id
  from public.organizations where slug = 'demo-clinic' limit 1;
  ...
  values (new.id, coalesce(requested_org_id, fallback_org_id),
          profile_name, coalesce(new.email,''), profile_role)
...
-- ── Profile seed trigger: auto-assign starter modules ────────────────────────

create or replace function public.seed_user_starter_data(
...
begin
  for rec in
    ...
    from public.training_modules tm
    where tm.organization_id = p_organization_id and tm.is_active = true
    limit 4
  loop
    insert into public.module_assignments
      (organization_id,user_id,module_id,status,score,
       assigned_at,due_date,completed_at)
    values (
      p_organization_id, p_user_id, rec.module_id,
      case rec.rn
      ...
      case rec.rn
        when 1 then now() + interval '11 months'
    ...
    if rec.rn = 1 then
      select c.id into existing_cert_id
      from public.certifications c
      where c.organization_id = p_organization_id
        and c.user_id = p_user_id and c.module_id = rec.module_id limit 1;
      if existing_cert_id is null then
        insert into public.certifications
          (organization_id,user_id,module_id,certificate_no,issued_at,expires_at)
        values (
          p_organization_id, p_user_id, rec.module_id,
  ...
  insert into public.notifications
    (organization_id,user_id,kind,title,message,created_at)
  select p_organization_id, p_user_id, v.kind, v.title, v.message, v.created_at
  from (values
    ('assignment'::text,'Welcome to MedCompliance',
     'Your starter compliance modules are ready. Begin with HIPAA Privacy Rule Fundamentals.',
     now() - interval '1 day'),
    ('due_soon'::text,'Training Due Soon',
     'One assigned module is due in the next 7 days. Complete it to keep your compliance score high.',
     now() - interval '12 hours'),
    ('renewal'::text,'Certification Active',
     'A compliance certificate is now active on your profile.',
     now() - interval '2 hours')
  ) as v(kind,title,message,created_at)
  where not exists (
    select 1 from public.notifications n
    where n.organization_id = p_organization_id
      and n.user_id = p_user_id and n.title = v.title
  );
  insert into public.audit_logs
    (organization_id,actor_user_id,action,entity_type,entity_id,metadata,created_at)
  select p_organization_id, p_user_id, 'profile_seeded', 'profiles',
         p_user_id::text, jsonb_build_object('seeded',true), now()
  where not exists (
...
-- Profiles: see all in org; update only self
...
-- Audit logs: managers read; all authenticated insert
